...
model.add(
    tf.keras.layers.Cropping2D(cropping=((13, 13), (20, 41)), input_shape=input_shape)
)
...
def calculating_metrics(
    ...
    if dataset_name == "testing":
        pkl_file = (
            "D:/Models/PPM/MimicPerformAF_10fold/"
            + "Part6"
            + "/results/"
            + dataset_name
            + ".pkl"
...
two_plotting_metrics(
    accuracy, val_accuracy, output_path, "train_accuracy", "validation_accuracy"
)
...
validation_metrics = calculating_metrics(
    train_path, validation, validation, model, results_path
)
